...
OUT_MESH_PROJECT=out-mesh
GW_PROJECT=gateways

ps1_bg_color=${c['bg_CYAN']}
ps1_color=${c['ORANGE']}
ps1_demo="GW Injection Demo >> "
...
pi "# Lets check the gateway pods"
pe "oc get pods -n $GW_PROJECT -o yaml"
...
pi "# We now want to create a project out of the mesh"
pe "oc new-project $OUT_MESH_PROJECT"

pi "# Now we deploy a dummy application"
app_manifest=$(cat manifests/out-mesh-deployment.yaml)
pi "cat << EOF | oc create -n $OUT_MESH_PROJECT -f -"
echo "$app_manifest"
echo EOF
echo "$app_manifest" | oc create -n $OUT_MESH_PROJECT -f -

pi "# Check the connection to the services from the pod out of the mesh:"
pi "# echo-8081.${PROJECT_8000}.svc.cluster.local 8081"
pi "# echo-8082.${PROJECT_8000}.svc.cluster.local 8082"
pi "# echo-8083.${PROJECT_8000}.svc.cluster.local 8083"
pi "# echo-9091.${PROJECT_9000}.svc.cluster.local 9091"
pi "# echo-9092.${PROJECT_9000}.svc.cluster.local 9092"
pi "# echo-9093.${PROJECT_9000}.svc.cluster.local 9093"

set +e
pe "oc rsh -n $OUT_MESH_PROJECT deploy/out-mesh"
set -e

pi "Lets create the Network Policy into the ${PROJECT_9000} namespace"

np_manifest=$(cat manifests/out-mesh-policy.yaml)
pi "cat << EOF | oc create -n ${PROJECT_9000} -f -"
echo "$np_manifest"
echo EOF
echo "$np_manifest" | oc create -n ${PROJECT_9000} -f -
